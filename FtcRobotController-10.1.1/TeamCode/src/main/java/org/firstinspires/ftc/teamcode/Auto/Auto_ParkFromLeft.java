...
public class Auto_ParkFromLeft extends LinearOpMode {
    ...
    static final double     DRIVE_GEAR_REDUCTION    = 80.0 ;
    ...
    public void runOpMode() {
        ...
        Drive_FrontLeft.setDirection(DcMotor.Direction.FORWARD);
        Drive_RearLeft.setDirection(DcMotor.Direction.FORWARD);
        Drive_FrontRight.setDirection(DcMotor.Direction.REVERSE);
        Drive_RearRight.setDirection(DcMotor.Direction.REVERSE);
        ...
        Drive_FrontLeft.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        Drive_FrontRight.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        Drive_RearLeft.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        Drive_RearRight.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
    ...
    */
    public void driveStraight(double maxDriveSpeed,
        ...
        if (opModeIsActive()) {
            ...
            Drive_RearLeft.setTargetPosition(rearLeftTarget);
            Drive_FrontRight.setTargetPosition(frontRightTarget);
            Drive_RearRight.setTargetPosition(rearRightTarget);

            Drive_FrontLeft.setMode(DcMotor.RunMode.RUN_TO_POSITION);
            Drive_RearLeft.setMode(DcMotor.RunMode.RUN_TO_POSITION);
            Drive_FrontRight.setMode(DcMotor.RunMode.RUN_TO_POSITION);
            Drive_RearRight.setMode(DcMotor.RunMode.RUN_TO_POSITION);
            ...
            while   (
                    opModeIsActive() &&
                    (
                       Drive_FrontLeft.isBusy() && 
                       Drive_FrontRight.isBusy() &&
                       Drive_RearLeft.isBusy() &&
                       Drive_RearRight.isBusy()
                    )
                    ) 
                       {
    ...
    public void moveRobot(double drive, double turn) {
        ...
        double leftBias = 1.0;  // Reduce to <1.0 if left motors are stronger
        double rightBias = 0.0; // Reduce to <1.0 if right motors are stronger

        Drive_FrontLeft.setPower(leftSpeed * leftBias);
        Drive_RearLeft.setPower(leftSpeed * leftBias);
        Drive_FrontRight.setPower(rightSpeed * rightBias);
        Drive_RearRight.setPower(rightSpeed * rightBias);
    }
    ...
    private void sendTelemetry(boolean straight) {
        ...
        if (straight) {
            ...
            telemetry.addData("Actual Pos L:R",  "%7d:%7d",      Drive_FrontLeft.getCurrentPosition(),
                    Drive_FrontRight.getCurrentPosition());
        } else {
